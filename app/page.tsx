export default function Home() {
  return (
    <>
      <head>
        <title>JEE Mains 2025 Marks vs Percentile - Accurate Predictor</title>
        <meta name="description" content="Check JEE Mains 2025 marks vs percentile chart. Get your predicted percentile and rank instantly. Trusted by 25K+ students." />
        <meta name="keywords" content="JEE Mains 2025, marks vs percentile, JEE percentile predictor, response sheet analysis, JEE score vs rank, JEE marks to percentile" />
      </head>
      <main>
        <h1>JEE Mains 2025: Marks vs Percentile Predictor</h1>
        <p>Get accurate JEE Mains 2025 percentile predictions using real-time data analysis.</p>
        <p>Trusted by 25K+ students, our calculator helps you analyze your response sheet and rank.</p>
        <p>Redirecting to <a href="https://www.prodijee.in">Prodijee.in</a> in 5 seconds...</p>
        <script>
          {`setTimeout(() => { window.location.href = "https://www.prodijee.in"; }, 5000);`}
        </script>
      </main>
    </>
  );
}
